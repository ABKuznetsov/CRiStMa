...
The structural graph itself remains a symmetry quotient graph:

```text
StructuralUnitOrbit
├─ source_shell_orbit_id | None
├─ source_polyhedron_orbit_id | None
├─ constituent_site_refs
└─ multiplicity_in_reference_cell

StructuralConnectionOrbit
├─ endpoint unit-orbit IDs
├─ exact PeriodicSymmetryRelation
├─ SHARED_VERTEX / SHARED_EDGE / SHARED_FACE / DIRECT_CONTACT
├─ source resolved-contact-orbit IDs
└─ multiplicity_in_reference_cell
```

A planar resolved shell such as BO3 remains a `COORDINATION` unit even when it
does not define a three-dimensional polyhedron. One physical connection cannot
simultaneously produce a `DIRECT_CONTACT` edge and a `SHARED_*` edge.
Representations only filter this graph by interaction layer and shell role;
they do not recalculate chemistry or connectivity.

Periodic dimensionality is the rank of the exact integer translation subgroup
generated by affine cycle closures. Rank 0, 1, 2, and 3 mean respectively a
finite component, chain, layer, and framework. Cartesian geometry never changes
this rank. The integer basis is primitive: collinear generators `(2,0,0)` and
`(3,0,0)` reduce to `(1,0,0)`.

`StructuralBlock` is a projection of one maximal quotient-graph component. It
stores unit-orbit IDs, connection-orbit IDs, exact periodic generators, rank,
